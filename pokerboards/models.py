...
class Pokerboard(commons_models.SoftDelete):
    FIBONACCI = 0
    ODD = 1
    EVEN = 2
    SERIAL = 3
    ...
    deck = ArrayField(models.DecimalField(
        max_digits=pokerboards_constant.ESTIMATE_MAX_DIGITS,
        decimal_places=pokerboards_constant.ESTIMATE_DECIMAL_PLACES,
        validators=[
            MaxValueValidator(pokerboards_constant.ESTIMATE_MAX_VALUE),
            MinValueValidator(pokerboards_constant.ESTIMATE_MIN_VALUE)
        ]), size=pokerboards_constant.DECK_MAX_SIZE)
...
class UserPokerboard(commons_models.SoftDelete):
    PLAYER = 0
    SPECTATOR = 1
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                             on_delete=models.CASCADE, related_name="pokerboards")
    pokerboard = models.ForeignKey(Pokerboard, on_delete=models.CASCADE)
    role = ArrayField(models.PositiveSmallIntegerField(
        choices=ROLE, default=PLAYER))
...
class Ticket(commons_models.Timestamp):
    ...
    def __str__(self):
        return "{} -> {}".format(self.pokerboard.name, self.ticket_id)
